...
struct mfd_demo_child_two_data {
	int demo_value;
};
...
{
	struct mfd_demo_child_two_data *data = dev_get_drvdata(dev);
...
{
	struct mfd_demo_child_two_data *data;
	int ret;
	...
	if (ret) {
		dev_err(&pdev->dev, "child-two failed to create sysfs node: %d\n", ret);
		return ret;
	}
